<template>
  <div
      v-if="toast.message"
      id="toast-success"
      class="shadow-neutral-700 z-50 absolute top-20 right-80 flex items-center p-4 mb-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800"
      role="alert"
  >
    <div
        v-if="
    ...
    <div
        v-else-if="toast.level === toastLevel.SEVERE"
    ...
    <div
        v-else-if="toast.level === toastLevel.WARN"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => setTimeout(() => emit('clear-toast'), props.duration))

    return {
      clearToast,
      toastLevelClasses: computed(() => ({
        severe: props?.toast?.level === ToastLevel.SEVERE,
        warn: props?.toast?.level === ToastLevel.WARN,
        success: props?.toast?.level === ToastLevel.SUCCESS,
      })),
      toastLevel: computed(() => ToastLevel),
    }
  },
})
</script>
