defmodule Api.AccountTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2022-10-23 14:58:00]}

      assert {:ok, %Clock{} = clock} = Account.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2022-10-23 14:58:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2022-10-24 14:58:00]}

      assert {:ok, %Clock{} = clock} = Account.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2022-10-24 14:58:00]
    end
  ...
  describe "workingtimes" do
    ...
    test "create_workingtime/1 with valid data creates a workingtime" do
      valid_attrs = %{end: ~N[2022-10-23 14:59:00], start: ~N[2022-10-23 14:59:00]}

      assert {:ok, %Workingtime{} = workingtime} = Account.create_workingtime(valid_attrs)
      assert workingtime.end == ~N[2022-10-23 14:59:00]
      assert workingtime.start == ~N[2022-10-23 14:59:00]
    end
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      update_attrs = %{end: ~N[2022-10-24 14:59:00], start: ~N[2022-10-24 14:59:00]}

      assert {:ok, %Workingtime{} = workingtime} = Account.update_workingtime(workingtime, update_attrs)
      assert workingtime.end == ~N[2022-10-24 14:59:00]
      assert workingtime.start == ~N[2022-10-24 14:59:00]
    end
