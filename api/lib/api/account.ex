defmodule Api.Account do
  ...
  """
  def get_teams(%{"team" => team}) do
    Repo.all(from u in User,
             where: ^team in u.teams)
  end

  def get_listuser(%{"username" => username, "email" => email}) do
    Repo.all(from u in User,
      where: u.username == ^username and u.email == ^email)
  end

  def get_listuser(%{"email" => email}) do
    Repo.all(from u in User,
      where: u.email == ^email)
  end

  def get_listuser(%{"username" => username}) do
    Repo.all(from u in User,
  ...
  """
  def get_clock!(userId), do: Repo.get_by!(Clock, [user: userId])

  def is_exist_clock!(userId), do: Repo.exists?(from c in Clock, where: c.user == ^userId)
  @doc """
  Creates a clock.
  ...
  #def update(conn, %{"user" => user, "clock_status" => clock_params}) do
  #  # Find a user by its username
  #  clock = Repo.get_by!(Clock, user: user)
  #  # Generate a changeset using existing user and incoming user_params
  #  changeset = User.changeset(user, %{status: false})
  #  # Attempt to save changeset to the database
  #  case Repo.update(changeset) do
  #    {:ok, user} ->
  #      conn
  #      |> put_flash(:info, "User updated successfully.")
  #      |> redirect(to: user_path(conn, :show, user.user))
  #    {:error, changeset} ->
  ...
  def get_listworkingtimes(%{"userId" => userId, "start" => start, "end" => endTime}) do
    Repo.all(from w in Workingtime,
      where: w.user == ^userId and w.start == ^start and w.end == ^endTime)
  end

  def get_listworkingtimes(%{"userId" => userId, "start" => start}) do
    Repo.all(from w in Workingtime,
      where: w.user == ^userId and w.start == ^start)
  end

  def get_listworkingtimes(%{"userId" => userId, "end" => endTime}) do
    Repo.all(from w in Workingtime,
      where: w.user == ^userId and w.end == ^endTime)
  end

  def get_listworkingtimes(%{"userId" => userId}) do
    Repo.all(from w in Workingtime,
      where: w.user == ^userId)
  end

  def get_listworkingtimes() do
